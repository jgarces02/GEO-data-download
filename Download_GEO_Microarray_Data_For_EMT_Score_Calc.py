				...
				if probe in GeneIDMap[gene]:
					...
					for val in l[1:]:
						if 'null' in val:
							...
							expressionValue = float(val.strip())
							if expressionValue < 0.0 and takeLogFlag == 1:
								expressionValue = 0.0
							if takeLogFlag == 1:
								expressionValue = log(expressionValue + 1.0, 2)
...
U133Map = {}
for probe in availProbes:
	U133Map[probe] = []
with open('data/GPL570-55999.txt', 'r') as f:
	for line in f:
		l = line.strip().split('\t')
		probe = l[0].strip()
		if probe in availProbes and len(l) > 10:
			genes = l[10].strip().split('///')
			for gene in genes:
				if gene in availGenes:
					U133Map[probe].append(gene)
...
usedNormalizersProbes = []
usedNormalizersProbeIndices = []
usedNormalizerGenes = []
for probe in normalizerProbes:
	if probe in U133Map:
		for gene in U133Map[probe]:
			if gene not in usedNormalizerGenes:
				usedNormalizerGenes.append(gene)
				usedNormalizersProbes.append(probe)
...
GSENormalizerData = {}
probeCounts = {}
for gene in usedNormalizerGenes:
	probeCounts[gene] = 0
					...
					for val in l[1:]:
						if 'null' in val:
							...
							if expressionValue < 0.0 and takeLogFlag == 1:
								expressionValue = 0.0
							if takeLogFlag == 1:
								expressionValue = log(1.0 + expressionValue, 2)
							probeData.append(expressionValue)
					if gene in GSENormalizerData and len(GSENormalizerData[gene]) > 0:
						for i in range(len(probeData)):
							GSENormalizerData[gene][i] += probeData[i]
					else:
						GSENormalizerData[gene] = []
						for val in probeData:
							GSENormalizerData[gene].append(val)
		if 'ID_REF' in line:
			l = line.strip().split('\t')
			patientNum = len(l)
			flag = 1

for gene in GSENormalizerData:
	for i in range(len(GSENormalizerData[gene])):
		GSENormalizerData[gene][i] /= probeCounts[gene]

for gene in usedNormalizerGenes:
	GSEOutFile.write(gene + ' ')
	for val in GSENormalizerData[gene]:
		GSEOutFile.write(str(val) + ' ')
	GSEOutFile.write('\n')
